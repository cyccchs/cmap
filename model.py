...
class MultiAgentRecurrentAttention(nn.Module):
    def __init__(self, ckpt_dir, batch_size, agent_num, h_g, h_l, k, s, glimpse_size, glimpse_num, c, hidden_size, loc_dim, std, device):
        ...
        self.rnn = networks.CoreNetwork(batch_size, hidden_size, device)
    ...
    def reset(self):
        ...
        init_c = torch.zeros(self.batch_size, 256, dtype=torch.float32, device=self.device, requires_grad=True) #lstm size

        return init_l_list, init_h, init_c

    def ram_loop(self, img, h_prev, c_prev, l_prev, i, train_terminate, sampling):
        ...
        h_t, c_t = self.rnn(z_t, h_prev, c_prev)
        ...
        for j in range(self.agent_num):
            log_pi, l_t = self.agents[j].location(s_t[j], sampling)
        ...
        #return h_t, c_t, prob, l_list, b_t, log_pi_t, log_probas, alpha, last
        return h_t, c_t, l_list, b_t, log_pi_t, log_probas, alpha, last


    def forward(self, img, train_terminate, sampling=True):
        ...
        for i in range(self.glimpse_num):
            #h_t, c_t, prob, l_t, b_t, log_pi_t, log_probas, alpha, last = self.ram_loop(img, h_prev, c_prev, l_prev, i, train_terminate, sampling)
            h_t, c_t, l_t, b_t, log_pi_t, log_probas, alpha, last = self.ram_loop(img, h_prev, c_prev, l_prev, i, train_terminate, sampling)
        ...
        return l_list, b_list, log_pi_list, log_probas, alpha, g_num
        """
